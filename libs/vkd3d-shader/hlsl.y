        ...
        const struct parse_initializer *params, const struct vkd3d_shader_location *loc)
{
    ...
    {
        /* Control flow should never reach here at time of writing because
         * doubles aren't implemented yet, but this is futureproofing for
         * when they are implemented. */
        hlsl_fixme(ctx, loc, "frexp on doubles.");
        return false;
    }
    type = hlsl_get_numeric_type(ctx, type->class, HLSL_TYPE_FLOAT, type->dimx, type->dimy);
    uint_dim_type = hlsl_get_numeric_type(ctx, type->class, HLSL_TYPE_UINT, type->dimx, type->dimy);
    int_dim_type = hlsl_get_numeric_type(ctx, type->class, HLSL_TYPE_INT, type->dimx, type->dimy);
    bool_dim_type = hlsl_get_numeric_type(ctx, type->class, HLSL_TYPE_BOOL, type->dimx, type->dimy);
